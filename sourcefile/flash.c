...
// for only 28F008SA flash.
static int flash_command_8bit(uint32_t base, uint8_t cmd)   
	{
	uint8_t buf[4];
...
//	fprintf(stderr, "flash command 0x%02X\n", cmd);
 ...
 // for 28F010/020 flash, zeroing before erasing
...
//	printf("Flash ID2: 0x%08x\n", buf);
	...
	Ven_Dev_ID = ((Device_ID>>8) & 0xFF) | (Vendor_ID & 0xFF00);   // first  byte is dev_ID ,upper byte is manufacture ID)
										//0x01a7  = 28F010 (AMD)				
										//0x012a  = 28F020 (AMD)				
										//0x89b4  = 28F010 (intel)				
										//0x89bd  = 28F020 (intel)				
	...
	printf("\nFlash ID : 0x%04x", Ven_Dev_ID);	

	switch (Ven_Dev_ID){
	case 0x01a7: { printf("\nFlash Type : AMD 28F010  "); break;}
	case 0x012a: { printf("\nFlash Type : AMD 28F020  "); break;}
	case 0x89a0: { printf("\nFlash Type : 28F016SA"); break;}
	case 0x89a2: { printf("\nFlash Type : 28F008SA"); break;}
	case 0x89b4: { printf("\nFlash Type : Intel 28F010  "); break;}
	case 0x89bd: { printf("\nFlash Type : Intel 28F020  "); break;}
	case 0xb0d0: { printf("\nFlash Type : 28F160S5"); break;}
...
// bytewise programming, there is highly optimizied for flashing speedup  (using small 68k code or block programming)
