...
//bytewise flashing program (flash is slow with write (up to 100µs cycle))
static int flash_program_28F0x0(uint32_t base, uint32_t data, uint32_t flash_size)
{
	...
	if (base == 0x1000000) 	{		// faster algorithm		// only at 0x1000000 extra command, why : base-0x4 at frist = 0x0FFFFFF, flash1 CE disabled)
		if (flash_command(base, 0x40) == -1)
			goto out;	
		if (write_memory((base), (uint8_t *)&data, sizeof(data)) == -1)
			goto out;
	}
	else if (base == (0x1000000 + flash_size)) {			// only at 0x1800000 extra command, why : base-0x4 at frist = 0x0FFFFFF, flash2 CE disabled)
		if (flash_command((base -flash_size), 0xFF) == -1)  // flash at 0x1000000 reset
			goto out;
	...
	else if (base == (0x1000000 + (flash_size*2))) {			// only at 0x110000 extra command, why : base-0x4 at frist = 0x0FFFFFF, flash3 CE disabled)
		if (flash_command((base - flash_size), 0xFF) == -1)		// flash at 0x1800000 reset
			goto out;
		...
		if (write_memory((base-0x4), (uint8_t *)&data_array, sizeof(data_array)) == -1) 
			goto out;
	}
...
// flash & verify algorithm isn't using, because  it is very slow)
...
int 		try = tries;
uint32_t	buf = 0;
uint32_t	safe_erase = 0;  // added for pervent of overearsing 0x00 = no erase , 0xFF = erase 
uint32_t 	size;
uint32_t 	addr;
...
// for flash erase FF-verify  (don't modified it, otherwise flash will be  unuseable/destroyed with overearsing )
